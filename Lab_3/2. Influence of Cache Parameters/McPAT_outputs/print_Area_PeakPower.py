...
def main():
    ...
    getEnergy(args[0], args[1])
    #print "energy is %f mJ" % energy
    

def getEnergy(mcpatoutputFile, statsFile):
    peak_Power, area, dcache_area, icache_area, l2cache_area = readMcPAT(mcpatoutputFile)
    #runtime = getTimefromStats(statsFile)
    #energy = (leakage + dynamic)*runtime
    print "Peak Power: %f W \nTotal Area: %f mm^2 \nL2 Cache Area: %f mm^2 \nData Cache Area: %f mm^2 \nInstruction Cache Area: %f mm^2" % (peak_Power, area, l2cache_area, dcache_area, icache_area)
    #return energy*1000
...
def readMcPAT(mcpatoutputFile):
    ...
    peak_Power = p.getValue(['Processor:', 'Peak Power'])
    area = p.getValue(['Processor:', 'Area'])
    dcache_area = p.getValue(['Core:', 'Load Store Unit:', 'Data Cache:', 'Area'])
    icache_area = p.getValue(['Core:', 'Instruction Fetch Unit:', 'Instruction Cache:', 'Area'])
    l2cache_area = p.getValue(['Processor:', 'Total L2s:', 'Area'])
    peak_Power = re.sub(' W','', peak_Power) 
    area = re.sub(' mm\^2','', area) 
    dcache_area = re.sub(' mm\^2','',dcache_area)
    icache_area = re.sub(' mm\^2','',icache_area)
    l2cache_area = re.sub(' mm\^2','',l2cache_area)
    return (float(peak_Power), float(area), float(dcache_area), float(icache_area), float(l2cache_area))
